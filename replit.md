...
- **Owner Builder** (`/build`, Feb 2026): Public-facing conversion funnel at `client/src/features/owner/OwnerWizard.tsx`. Self-contained component (no WizardContext dependency) that lets visitors build custom QR products without authentication. Key differences from member wizards: no channel step, cost framing (blue running cost badge) instead of earnings, minimum tier enforcement via `?type=basic|plus|canvas|play|compose` URL param, Canvas/Play/Compose locked behind membership gate in type picker. After building, shows cost summary breakdown and member conversion pitch ("Turn This Into Income"). Reuses shared wizard step components. Page wrapper at `client/src/pages/build.tsx`, public route (no auth gate).
