...
function StoreProductCard({ product }: { product: StoreProduct }) {
  ...
  const handleAddToCart = async (e: React.MouseEvent) => {
    ...
    try {
      const addToCartRes = await fetch(`/api/store/product/${product.id}/add-to-cart`, {
        method: "POST",
  ...
  return (
    ...
    >
      ...
      <CardContent className="flex-1 p-4 flex flex-col gap-3">
        <div>
          <Link href={`/shop/product/${product.id}`}>
            <h3 className="font-semibold text-lg line-clamp-2 cursor-pointer hover:underline" data-testid={`text-product-name-${product.id}`}>
              {product.name}
            </h3>
          </Link>
            ...
            <Select value={selectedSize} onValueChange={setSelectedSize}>
              ...
              <SelectContent>
                {availableSizes.map((size) => (
                  <SelectItem key={size} value={size} data-testid={`option-size-${size.toLowerCase()}`}>
                    {size}
                  </SelectItem>
                ))}
              </SelectContent>
