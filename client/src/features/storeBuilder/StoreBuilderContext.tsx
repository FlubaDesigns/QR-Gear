...
interface StoreBuilderContextValue {
  productPackage: ProductPackage | null;
  setProductPackage: (p: ProductPackage | null) => void;
  ...
  setGraphicSize: (size: string) => void;
  selectedStoreType: StoreType;
  setSelectedStoreType: (t: StoreType) => void;
  ...
  setWantsToChangeDestination: (v: boolean) => void;
  filteredStores: PartnerStore[];
  selectedStore: PartnerStore | undefined;
  channels: string[];
  showAddStore: boolean;
  setShowAddStore: (v: boolean) => void;
  showAddChannel: boolean;
  setShowAddChannel: (v: boolean) => void;
  newStoreName: string;
  setNewStoreName: (n: string) => void;
  newChannelName: string;
  setNewChannelName: (n: string) => void;
  ...
  setThumbnailLightbox: (url: string | null) => void;
  ...
  handleTemplateSelect: (packetId: string) => void;
  handleRefreshPacket: () => void;
  handleClearAfterAssign: () => void;
  handleViewInStore: () => void;
  handleChannelSelect: (channel: string) => Promise<void>;
  fetchTemplates: () => Promise<TemplateItem[]>;
}

const StoreBuilderContext = createContext<StoreBuilderContextValue | null>(null);

export function useStoreBuilder(): StoreBuilderContextValue {
  const ctx = useContext(StoreBuilderContext);
  if (!ctx) throw new Error("useStoreBuilder must be used inside StoreBuilderProvider");
  return ctx;
}
...
export function StoreBuilderProvider({ children }: { children: React.ReactNode }) {
  ...
  const fetchTemplates = useCallback(async (): Promise<TemplateItem[]> => {
  ...
  useEffect(() => {
    if (selectedStoreId && stores.length > 0 && !selectedStoreType) {
      ...
      if (store) setSelectedStoreType(store.isInternal ? "internal" : "external");
    }
  ...
  useEffect(() => {
    if (productPackage) {
      ...
      if (isEditMode && !originalConfiguration) setOriginalConfiguration(initialConfig);
    }
      ...
      adminFetch<any>(`/packets/${packetId}`)
        .then(data => {
          if (data.success && data.packet) {
            ...
            if (!urlChannel && packet.channelName) setSelectedChannel(packet.channelName);

            if (packet.storeId && packet.channelName) {
              adminFetch<any>(`/stores/${packet.storeId}/channels/${packet.channelName}/collections`)
                .then(d => setExistingCollections(d.collections || []))
            ...
            if (!packet.priorityMockupUrl && packet.blueprintId && packet.compositeUrl) {
              ...
              adminFetch<any>("/mockup/priority", {
                method: "POST",
                json: { blueprintId: packet.blueprintId, printProviderId: packet.printProviderId || 99, colorName, colorHex, placement: placement + "-center", artworkUrl: packet.compositeUrl, qrSize },
              })
                .then(mockupData => {
        ...
        .finally(() => setIsLoadingPacket(false));
      return;
    }
  ...
  const toggleColor = (colorName: string) => {
    setConfiguration(prev => {
      const newColors = new Set(prev.enabledColors);
      if (newColors.has(colorName)) {
        if (newColors.size > 1) newColors.delete(colorName);
      } else {
        newColors.add(colorName);
      }
      let newDefault = prev.defaultColor;
      if (!newColors.has(newDefault)) newDefault = Array.from(newColors)[0] || "";
      return { ...prev, enabledColors: newColors, defaultColor: newDefault };
    });
  };

  const toggleSize = (size: string) => {
    setConfiguration(prev => {
      const newSizes = new Set(prev.enabledSizes);
      if (newSizes.has(size)) {
        if (newSizes.size > 1) newSizes.delete(size);
      } else {
        newSizes.add(size);
      }
      return { ...prev, enabledSizes: newSizes };
    });
  };
  ...
  const handleCreateStore = async () => {
    ...
    try {
      ...
      if (result.id) { setSelectedStoreId(result.id); setSelectedChannel(null); }
    } catch (err: unknown) {
  ...
  const handleAssign = async () => {
    ...
    try {
      ...
      if (!result.success) { setSaveStatus({ type: "error", message: result.message }); return; }
  ...
  const handleRefreshPacket = () => {
    ...
    if (currentPacketId) {
      setProductPackage(null);
      setTimeout(() => navigate(`/admin/store-builder?packetId=${currentPacketId}`), 50);
    }
  };
  ...
  const handleClearAfterAssign = () => {
    ...
    setOriginalPacketId(null);
    setOriginalConfiguration(null);
    setConfiguration({ enabledColors: new Set<string>(), enabledSizes: new Set<string>(), selectedGraphicSize: "medium", defaultColor: "" });
  ...
  return (
    <StoreBuilderContext.Provider value={{
      productPackage, setProductPackage, isLoadingPacket, isEditMode,
      configuration, toggleColor, toggleSize, setDefaultColor, setGraphicSize,
      selectedStoreType, setSelectedStoreType,
      selectedStoreId, setSelectedStoreId,
      selectedChannel, setSelectedChannel,
      selectedCollection, setSelectedCollection,
      existingCollections, wantsToChangeDestination, setWantsToChangeDestination,
      ...
      thumbnailLightbox, setThumbnailLightbox,
      templatePickerOpen, setTemplatePickerOpen,
      mockups, previewImageUrl, packetThumbnails, defaultColorHex,
      handleCreateStore, handleCreateChannel, handleAssign,
      handleTemplateSelect, handleRefreshPacket, handleClearAfterAssign,
      handleViewInStore, handleChannelSelect, fetchTemplates,
    }}>
      {children}
