...
export function SourceModule() {
  ...
  const [pickerMode, setPickerMode] = useState<PickerMode>("templates");
  ...
  const handleAssetSelect = (asset: LibraryAsset) => {
    if (pickerMode === "backgrounds" || pickerMode === "cropped") {
      loadBackground({
        url: asset.proxyUrl || asset.url || "",
        isClipped: pickerMode === "cropped" || asset.type === "cropped",
      });
    }
  };
                ...
                <div className="p-3 border rounded-lg bg-muted/50 space-y-3">
                  ...
                  <div className="flex gap-2">
                    ...
                    <Button 
                      variant="secondary" 
                      size="sm"
                      onClick={() => openPicker("backgrounds")}
                  ...
                  {state.loadedBackground && (
                    <div className="mt-3 p-2 border rounded bg-background">
                      <p className="text-xs text-muted-foreground mb-2">Selected Background:</p>
                      <div className="flex items-center gap-3">
                        ...
                        <div className="text-sm">
                          <p className="font-medium">{state.loadedBackground.isClipped ? "Clipped" : "Full"}</p>
                        </div>
                      </div>
                    </div>
                  )}
