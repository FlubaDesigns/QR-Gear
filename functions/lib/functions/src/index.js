...
const INTERNAL_TO_PRINTFUL_DTF = {
...
// ============ ALLOWED PRODUCTS ============
app.get('/admin/stores/:storeId/allowed-products', requireAdmin, async (req, res) => {
    try {
        const { storeId } = req.params;
...
app.post('/admin/stores/:storeId/allowed-products', requireAdmin, async (req, res) => {
    try {
        const { storeId } = req.params;
...
app.get('/admin/fulfillment-providers', requireAdmin, async (_req, res) => {
    try {
...
app.get('/admin/pricing-settings', requireAdmin, async (_req, res) => {
    try {
        const doc = await db.collection("testSettings").doc("pricing").get();
        const defaultSizeUpcharges = { 'S': 0, 'M': 2, 'L': 4, 'XL': 6, '2XL': 8, '3XL': 10, '4XL': 12 };
        const defaultBrandLabelPricing = { printifyInside: 0.55, printifyOutside: 0.55, printfulInside: 0.99, printfulOutside: 2.49 };
...
app.get('/admin/packets', requireAdmin, async (_req, res) => {
    try {
...
app.get('/admin/packets/:packetId', requireAdmin, async (req, res) => {
    try {
...
app.patch('/admin/packets/:packetId', requireAdmin, async (req, res) => {
    try {
        const { packetId } = req.params;
...
app.delete('/admin/packets/:packetId', requireAdmin, async (req, res) => {
    try {
        const { packetId } = req.params;
        if (!packetId) {
            res.status(400).json({ error: "packetId is required" });
...
app.get('/admin/store-product-links', requireAdmin, async (_req, res) => {
    try {
...
app.post('/admin/store-product-links', requireAdmin, async (req, res) => {
    try {
...
app.get('/admin/stores/:storeId/channels/:channelId/products', requireAdmin, async (req, res) => {
    try {
        const { storeId, channelId } = req.params;
        if (!storeId || !channelId) {
            res.status(400).json({ error: "storeId and channelId are required" });
...
app.patch('/admin/store-product-links/:linkId', requireAdmin, async (req, res) => {
    try {
...
app.delete('/admin/store-product-links/:linkId', requireAdmin, async (req, res) => {
    try {
        const { linkId } = req.params;
        if (!linkId) {
            res.status(400).json({ error: "linkId is required" });
...
app.get('/admin/catalog/sync-status', requireAdmin, async (req, res) => {
    try {
...
app.post('/admin/catalog/sync', requireAdmin, async (req, res) => {
    try {
        if (!printifyClient.isConfigured) {
...
app.post('/admin/catalog/sync-printful', requireAdmin, async (req, res) => {
    try {
        if (!printfulClient.isConfigured) {
...
app.get('/admin/catalog/printful', requireAdmin, async (req, res) => {
    try {
...
app.get('/admin/catalog/printful/:productId', requireAdmin, async (req, res) => {
    try {
...
app.post('/admin/catalog/printful-mapping', requireAdmin, async (req, res) => {
    try {
...
app.get('/admin/catalog/printful-mappings', requireAdmin, async (_req, res) => {
    try {
...
app.get('/admin/catalog/placements', requireAdmin, async (req, res) => {
    try {
...
app.get('/admin/printify/catalog', requireAdmin, async (req, res) => {
    try {
        ...
        if (providerFilter === 'matched') {
            ...
            const printfulModels = new Set(allPrintfulRows.filter((pf) => pf.model && pf.model.trim() !== '').map((pf) => pf.model.trim().toLowerCase()));
        ...
        if (providerFilter === 'all' || providerFilter === 'printify' || providerFilter === 'matched') {
            ...
            for (const bp of blueprints) {
                ...
                categories[category].push({
                    ...
                    dualProvider: !!matchedPrintful, matchedProviderId: matchedPrintful ? `printful-${matchedPrintful.id}` : null,
                });
            }
        }
        ...
        const result = sortedCategories
            ...
            .map(name => ({
            name, items: categories[name].sort((a, b) => a.title.localeCompare(b.title)),
            count: categories[name].length,
            usaCount: categories[name].filter((i) => i.madeInUSA).length,
            printifyCount: categories[name].filter((i) => i.provider === 'printify').length,
            printfulCount: categories[name].filter((i) => i.provider === 'printful').length,
        }));
        const extraCategories = Object.entries(categories)
            .filter(([name]) => !sortedCategories.includes(name))
            .filter(([_, items]) => items.length > 0)
            .map(([name, items]) => ({
            name, items: items.sort((a, b) => a.title.localeCompare(b.title)),
            count: items.length, usaCount: items.filter((i) => i.madeInUSA).length,
            printifyCount: items.filter((i) => i.provider === 'printify').length,
            printfulCount: items.filter((i) => i.provider === 'printful').length,
        }));
...
app.get('/admin/printify/catalog/:blueprintId', requireAdmin, async (req, res) => {
    try {
        if (!printifyClient.isConfigured) {
            res.status(503).json({ error: "Printify API not configured" });
            return;
...
app.post('/admin/printify/catalog/batch-details', requireAdmin, async (req, res) => {
    try {
        if (!printifyClient.isConfigured) {
            res.status(503).json({ error: "Printify API not configured" });
            return;
        ...
        if (!Array.isArray(blueprintIds) || blueprintIds.length === 0) {
            res.status(400).json({ error: "blueprintIds array required" });
            return;
        }
        ...
        for (const blueprintId of limitedIds) {
            try {
                ...
                const usaProviders = providers.filter((p) => p.location?.country === 'US' || p.location?.country === 'USA');
                let variants = [];
                const selectedProvider = usaProviders[0] || providers[0];
                if (selectedProvider) {
                    try {
                        const variantData = await printifyClient.getVariants(blueprintId, selectedProvider.id);
...
app.get('/admin/catalog/printful-products', requireAdmin, async (_req, res) => {
    try {
...
app.get('/admin/catalog/printful-status', requireAdmin, async (_req, res) => {
    try {
        const productsSnapshot = await db.collection("printful_products").get();
        const variantsSnapshot = await db.collection("printful_variants").get();
        res.json({
...
app.get('/catalog/printful-products', async (_req, res) => {
    try {
        const productsSnapshot = await db.collection("printful_products").get();
        const products = productsSnapshot.docs.map(doc => ({ id: parseInt(doc.id) || doc.data().id, ...doc.data() }));
        const variantsSnapshot = await db.collection("printful_variants").get();
...
app.post('/admin/mockup/queue-process', requireAdmin, async (_req, res) => {
    try {
...
app.post('/admin/mockup/priority', requireAdmin, async (req, res) => {
    try {
...
app.post('/admin/content/upload', requireAdmin, async (req, res) => {
    try {
...
app.get('/admin/published-compose-items', requireAdmin, async (_req, res) => {
    try {
...
app.post('/admin/compose/publish', requireAdmin, async (req, res) => {
    try {
...
app.get('/admin/shelf-groups', requireAdmin, async (_req, res) => {
    try {
...
app.post('/admin/shelf-groups', requireAdmin, async (req, res) => {
    try {
...
app.patch('/admin/shelf-groups/:id', requireAdmin, async (req, res) => {
    try {
...
app.delete('/admin/shelf-groups/:id', requireAdmin, async (req, res) => {
    try {
...
app.get('/admin/build-shelf', requireAdmin, async (req, res) => {
    try {
...
app.post('/admin/build-shelf', requireAdmin, async (req, res) => {
    try {
...
app.patch('/admin/build-shelf/:id', requireAdmin, async (req, res) => {
    try {
        const updates = {};
...
app.delete('/admin/build-shelf/:id', requireAdmin, async (req, res) => {
    try {
...
app.post('/admin/pricing-settings/sync', requireAdmin, async (_req, res) => {
    try {
        const pricingDoc = await db.collection("testSettings").doc("pricing").get();
        const pricingSettings = pricingDoc.exists ? pricingDoc.data() : null;
        const markupPercent = pricingSettings?.markupPercent ?? 25;
...
app.post('/members/mockup/priority', requireAuth, async (req, res) => {
    try {
        const { blueprintId, printProviderId, colorName, colorHex, placement, artworkUrl, qrSize = "medium", fulfillmentProvider = "printify" } = req.body;
        if (!blueprintId || !colorName || !artworkUrl) {
            res.status(400).json({ error: "Missing required fields: blueprintId, colorName, artworkUrl" });
...
app.post('/members/generate-product-graphic', requireAuth, async (req, res) => {
    try {
        ...
        const productGraphicDataUrl = await cfGeneratePrintifyComposite(qrUrl, topText, bottomText, 1200, 1800, qrColor);
...
app.post('/public/generate-mockup', async (req, res) => {
    try {
        ...
        if (textLayoutChoice && textLayoutChoice !== '' && (headerStyle?.text || footerStyle?.text)) {
            ...
            const compositeDataUrl = await cfGeneratePrintifyComposite(qrUrl || 'https://example.com', topText, bottomText, 1200, 1800, qrColor);
...
app.post('/members/allowed-products', requireAdmin, async (req, res) => {
    try {
        const { products } = req.body;
        if (!Array.isArray(products)) {
            res.status(400).json({ error: "products must be an array" });
...
app.post('/admin/catalogs/:catalogId/duplicate', requireAdmin, async (req, res) => {
    try {
        const { catalogId } = req.params;
...
app.post('/admin/catalogs/:catalogId/bulk-copy', requireAdmin, async (req, res) => {
    try {
        const { catalogId } = req.params;
...
app.get('/admin/catalog-defaults', requireAdmin, async (req, res) => {
    try {
...
app.put('/admin/catalog-defaults', requireAdmin, async (req, res) => {
    try {
        ...
        await db.collection('systemSettings').doc('catalog-defaults').set({ defaultCatalogId: defaultCatalogId || null, updatedAt: new Date().toISOString() }, { merge: true });
...
app.get('/members/tier-products', async (req, res) => {
    try {
        ...
        if (printifyBlanks.length > 0 && printifyClient.isConfigured) {
            const missingRichDesc = printifyBlanks.filter((id) => {
            ...
            for (const blankId of missingRichDesc) {
                try {
                    ...
                    if (details && details.description) {
                        ...
                        if (bp)
                            bp.richDescription = details.description;
                        await db.collection('printify_blueprints').doc(String(numId)).set({ richDescription: details.description }, { merge: true });
                    }
                    await new Promise(r => setTimeout(r, 200));
                }
                catch (e) {
...
app.get('/members/:memberId/library', async (req, res) => {
    try {
        ...
        let query = db.collection('memberLibrary').where('memberId', '==', memberId).where('isActive', '==', true);
        if (assetType)
            query = query.where('assetType', '==', assetType);
...
app.get('/admin/dashboard/metrics', requireAdmin, async (_req, res) => {
    try {
...
app.get('/admin/email-templates', requireAdmin, async (_req, res) => {
    try {
...
app.put('/admin/email-templates/:id', requireAdmin, async (req, res) => {
    try {
...
app.delete('/admin/email-templates/:id', requireAdmin, async (req, res) => {
    try {
...
app.get('/admin/collections', requireAdmin, async (_req, res) => {
    try {
...
app.get('/admin/custom-designs', requireAdmin, async (_req, res) => {
    try {
...
app.post('/admin/custom-designs', requireAdmin, async (req, res) => {
    try {
...
app.put('/admin/custom-designs/:id', requireAdmin, async (req, res) => {
    try {
...
app.delete('/admin/custom-designs/:id', requireAdmin, async (req, res) => {
    try {
...
app.get('/admin/orchestration/master-products', requireAdmin, async (_req, res) => {
    try {
...
app.get('/admin/orchestration/master-products/:id', requireAdmin, async (req, res) => {
    try {
...
app.post('/admin/orchestration/master-products', requireAdmin, async (req, res) => {
    try {
...
app.patch('/admin/orchestration/master-products/:id', requireAdmin, async (req, res) => {
    try {
...
app.delete('/admin/orchestration/master-products/:id', requireAdmin, async (req, res) => {
    try {
...
app.get('/admin/orchestration/channel-configs', requireAdmin, async (_req, res) => {
    try {
...
app.get('/admin/orchestration/channel-configs/:channelType', requireAdmin, async (req, res) => {
    try {
...
app.post('/admin/orchestration/channel-configs', requireAdmin, async (req, res) => {
    try {
...
app.patch('/admin/orchestration/channel-configs/:channelType', requireAdmin, async (req, res) => {
    try {
...
app.post('/admin/orchestration/routing/route', requireAdmin, async (req, res) => {
    try {
...
app.post('/admin/orchestration/routing/batch', requireAdmin, async (req, res) => {
    try {
...
app.get('/admin/background-assets', requireAdmin, async (_req, res) => {
    try {
...
app.put('/admin/background-assets/:id', requireAdmin, async (req, res) => {
    try {
...
app.get('/admin/graphic-sets', requireAdmin, async (_req, res) => {
    try {
...
app.get('/admin/graphic-sets/:id', requireAdmin, async (req, res) => {
    try {
...
app.post('/admin/graphic-sets', requireAdmin, async (req, res) => {
    try {
...
app.put('/admin/graphic-sets/:id', requireAdmin, async (req, res) => {
    try {
...
app.delete('/admin/graphic-sets/:id', requireAdmin, async (req, res) => {
    try {
...
app.post('/pricing-settings/sync', requireAdmin, async (_req, res) => {
    try {
        const doc = await db.collection('testSettings').doc('pricing').get();
        if (!doc.exists) {
...
app.get('/admin/catalog/cost-sync-status', requireAdmin, async (_req, res) => {
    try {
...
app.get('/admin/catalog/sync-history', requireAdmin, async (_req, res) => {
    try {
...
app.get('/admin/hosting-tiers', requireAdmin, async (_req, res) => {
    try {
...
app.get('/admin/product-categories', requireAdmin, async (_req, res) => {
    try {
...
app.get('/admin/template-categories', requireAdmin, async (_req, res) => {
    try {
...
app.post('/admin/template-categories', requireAdmin, async (req, res) => {
    try {
...
app.put('/admin/template-categories/:id', requireAdmin, async (req, res) => {
    try {
...
app.delete('/admin/template-categories/:id', requireAdmin, async (req, res) => {
    try {
...
app.get('/admin/library', requireAdmin, async (_req, res) => {
    try {
...
app.post('/admin/upload-media', requireAdmin, async (req, res) => {
    try {
...
app.get('/dynamic-pages', requireAuth, async (req, res) => {
    try {
        const userId = req.user?.uid;
...
app.get('/dynamic-pages/:id', requireAuth, async (req, res) => {
    try {
...
app.post('/dynamic-pages', requireAuth, async (req, res) => {
    try {
        const userId = req.user?.uid;
...
app.post('/dynamic-pages/create', requireAuth, async (req, res) => {
    try {
        const userId = req.user?.uid;
        const { title, description, hostingTierId } = req.body;
        const slug = `dp-${Date.now().toString(36)}-${Math.random().toString(36).substring(2, 8)}`;
...
app.put('/dynamic-pages/:id', requireAuth, async (req, res) => {
    try {
        const { id } = req.params;
...
app.delete('/dynamic-pages/:id', requireAuth, async (req, res) => {
    try {
...
app.get('/dynamic-pages/:id/assets', requireAuth, async (req, res) => {
    try {
...
app.post('/dynamic-pages/:id/assets', requireAuth, async (req, res) => {
    try {
...
app.post('/dynamic-pages/:id/set-active', requireAuth, async (req, res) => {
    try {
...
app.post('/store-product-links', requireAdmin, async (req, res) => {
    try {
...
app.post('/admin/api-keys/test', requireAdmin, async (req, res) => {
    try {
...
app.post('/admin/channel-items/seed', requireAdmin, async (_req, res) => {
    try {
...
app.post('/admin/channel-items/:itemId/regenerate-assets', requireAdmin, async (req, res) => {
    try {
...
app.get('/admin/partner-stores/:id', requireAdmin, async (req, res) => {
    try {
...
app.patch('/admin/partner-stores/:id', requireAdmin, async (req, res) => {
    try {
...
app.post('/admin/partner-stores/:id/regenerate-key', requireAdmin, async (req, res) => {
    try {
...
app.get('/admin/email-templates/:id', requireAdmin, async (req, res) => {
    try {
...
app.patch('/admin/email-templates/:id', requireAdmin, async (req, res) => {
    try {
        await db.collection('email_templates').doc(req.params.id).update({ ...req.body, updatedAt: new Date().toISOString() });
        const doc = await db.collection('email_templates').doc(req.params.id).get();
        res.json({ id: doc.id, ...doc.data() });
...
app.post('/admin/background-assets/migrate', requireAdmin, async (_req, res) => {
    try {
...
app.put('/admin/fonts', requireAdmin, async (req, res) => {
    try {
...
app.get('/admin/provider-counts', requireAdmin, async (_req, res) => {
    try {
...
app.post('/admin/sync-blueprints-to-firestore', requireAdmin, async (_req, res) => {
    try {
...
app.post('/admin/sync-providers-to-firestore', requireAdmin, async (_req, res) => {
    try {
...
app.get('/admin/product-configs', requireAdmin, async (_req, res) => {
    try {
...
app.patch('/admin/products/:id/options', requireAdmin, async (req, res) => {
    try {
        const { id } = req.params;
...
app.get('/store/:storeType/:storeName', async (req, res) => {
    try {
            ...
            const products = await Promise.all(productsRaw.map(async (p) => {
                ...
                if (p.packetId) {
                    ...
                    if (pDoc.exists) {
                        const pkt = pDoc.data();
                        if (pkt.productId)
                            price = await getAuthoritativePrice(pkt.productId);
        ...
        let linksQuery = db.collection('storeProductLinks').where('storeId', '==', matchedStore.id);
        if (segment) {
        ...
        if (packetDoc.exists) {
            ...
            if (packet) {
                if (packet.textLayers?.length > 0) {
                    const titleLayer = packet.textLayers.find((l) => l.id === 'title' || l.id === 'header');
                    if (titleLayer?.text)
                        title = titleLayer.text;
                }
                if (packet.textLayers?.length > 0) {
                    const descLayer = packet.textLayers.find((l) => l.id === 'description' || l.id === 'footer');
                    if (descLayer?.text)
...
app.post('/auth/email-logout', async (req, res) => {
...
app.get('/admin/marketplace/stores', requireAdmin, async (req, res) => {
    try {
...
app.put('/admin/marketplace/stores/:storeId/config', requireAdmin, async (req, res) => {
    try {
        const { storeId } = req.params;
        const storeDoc = await db.collection('stores').doc(storeId).get();
        if (!storeDoc.exists) {
...
app.post('/admin/marketplace/stores/:storeId/listings', requireAdmin, async (req, res) => {
    try {
        const { storeId } = req.params;
        const storeDoc = await db.collection('stores').doc(storeId).get();
        if (!storeDoc.exists) {
...
app.get('/admin/marketplace/stores/:storeId/listings', requireAdmin, async (req, res) => {
    try {
        const { storeId } = req.params;
        const storeDoc = await db.collection('stores').doc(storeId).get();
        if (!storeDoc.exists) {
...
app.post('/admin/marketplace/stores/:storeId/listings/:listingId/push', requireAdmin, async (req, res) => {
    try {
...
app.patch('/admin/stores/:storeId', requireAdmin, async (req, res) => {
    try {
        const { storeId } = req.params;
        const storeDoc = await db.collection('stores').doc(storeId).get();
        if (!storeDoc.exists) {
