...
class PrintfulClient {
    ...
    async getVariantsByColor(productId, colorName) {
        ...
        return productData.variants.filter(v => v.color.toLowerCase() === lowerColor ||
...
async function generateMockupFromPrintful(request) {
    ...
    // Create mockup task
    const task = await printfulClient.createMockupTask(printfulProductId, [variantId], [{ placement: 'front', image_url: artworkUrl }], 'jpg', ['Lifestyle']);
    // Wait for completion
...
app.get('/test/fulfillment-providers', async (_req, res) => {
    try {
...
app.put('/test/products/:id', async (req, res) => {
    try {
        const { id } = req.params;
...
app.get('/test/stores/by-id/:storeId', async (req, res) => {
    try {
        const { storeId } = req.params;
...
app.post('/test/stores/:storeId/channels', async (req, res) => {
    try {
        const { storeId } = req.params;
        const { name } = req.body;
        if (!name || !name.trim()) {
...
// Delete a channel
app.delete('/test/stores/:storeId/channels/:channelId', async (req, res) => {
    try {
...
app.get('/test/stores/:type', async (req, res) => {
    try {
...
app.get('/test/partner-stores/:id/products', async (req, res) => {
    try {
        const { id } = req.params;
...
app.post('/test/partner-stores/:id/products', async (req, res) => {
    try {
        const { id } = req.params;
...
app.get('/test/printify/catalog/:blueprintId', async (req, res) => {
    try {
...
app.post('/test/stores/:storeId/channels/:channelId/products', async (req, res) => {
    try {
        const { storeId, channelId } = req.params;
        ...
        const assignedProducts = products.map((p) => ({
            id: p.id,
...
app.post('/test/templates', async (req, res) => {
    try {
        const { packetId, name, productId, blueprintId, printProviderId, artworkUrl, thumbnailUrl, qrContent, pricing, selectedSize, enabledColors, enabledSizes, defaultColor, isActive } = req.body;
        if (!packetId) {
...
app.get('/test/packets', async (req, res) => {
    try {
...
app.get('/test/packets/:packetId', async (req, res) => {
    try {
...
app.patch('/test/packets/:packetId', async (req, res) => {
    try {
        const { packetId } = req.params;
...
app.delete('/test/packets/:packetId', async (req, res) => {
    try {
        const { packetId } = req.params;
        if (!packetId) {
            res.status(400).json({ error: 'packetId is required' });
...
app.delete('/test/templates/:templateId', async (req, res) => {
    try {
...
app.get('/test/queue/status', async (_req, res) => {
    try {
...
app.post('/test/queue/process', async (req, res) => {
    try {
...
app.get('/test/store-product-links', async (req, res) => {
    try {
