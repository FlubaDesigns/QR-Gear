...
exports.COLOR_LABELS = (() => {
    const labels = {};
    for (const [name, code] of Object.entries(exports.COLOR_CODE_MAP)) {
        if (!labels[code])
            labels[code] = name;
    }
    return labels;
...
function getQrgSizeCode(sizeText) {
    ...
    if (exports.SIZE_CODE_MAP[normalized] !== undefined)
        return exports.SIZE_CODE_MAP[normalized];
    const upper = normalized.toUpperCase();
    for (const [key, code] of Object.entries(exports.SIZE_CODE_MAP)) {
        if (key.toUpperCase() === upper)
            return code;
    }
    const stripped = normalized.replace(/^(size|us|uk)\s+/i, '').trim();
    if (stripped && exports.SIZE_CODE_MAP[stripped] !== undefined)
        return exports.SIZE_CODE_MAP[stripped];
    for (const [key, code] of Object.entries(exports.SIZE_CODE_MAP)) {
...
function getQrgColorCode(colorText) {
    ...
    if (exports.COLOR_CODE_MAP[normalized] !== undefined)
        return exports.COLOR_CODE_MAP[normalized];
    const lower = normalized.toLowerCase();
    for (const [key, code] of Object.entries(exports.COLOR_CODE_MAP)) {
