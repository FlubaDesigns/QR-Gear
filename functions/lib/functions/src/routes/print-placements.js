...
function register(app) {
    ...
    app.get('/public/print-placements', async (req, res) => {
        try {
            const snap = await core_1.db.collection(COLLECTION).orderBy('sortOrder', 'asc').get();
    ...
    app.get('/admin/print-placements', middleware_1.requireAdmin, async (req, res) => {
        try {
            const snap = await core_1.db.collection(COLLECTION).orderBy('sortOrder', 'asc').get();
    ...
    app.get('/admin/print-placements/:id', middleware_1.requireAdmin, async (req, res) => {
        try {
            const doc = await core_1.db.collection(COLLECTION).doc(req.params.id).get();
            if (!doc.exists) {
    ...
    app.get('/admin/print-placements/lookup', middleware_1.requireAdmin, async (req, res) => {
        try {
            const { provider, providerName } = req.query;
            if (!provider || !providerName) {
                res.status(400).json({ error: 'provider and providerName query params are required' });
                return;
            }
            const snap = await core_1.db.collection(COLLECTION).get();
            ...
            if (!match) {
                res.status(404).json({ error: `No placement found for ${provider}:${providerName}` });
                return;
            }
    ...
    app.patch('/admin/print-placements/:id', middleware_1.requireAdmin, async (req, res) => {
        try {
            const allowed = ['displayName', 'description', 'dimensions', 'providers', 'sortOrder', 'isActive'];
            const update = { updatedAt: core_1.admin.firestore.FieldValue.serverTimestamp() };
            for (const key of allowed) {
                if (req.body[key] !== undefined)
                    update[key] = req.body[key];
            }
            const ref = core_1.db.collection(COLLECTION).doc(req.params.id);
    ...
    app.post('/admin/print-placements/seed', middleware_1.requireAdmin, async (req, res) => {
        try {
            ...
            for (const placement of SEED_PLACEMENTS) {
                const ref = core_1.db.collection(COLLECTION).doc(placement.internalName);
...
//# sourceMappingURL=print-placements.js.map