...
function register(app) {
    ...
    app.put('/admin/catalogs/:catalogId/blank-tier', middleware_1.requireAdmin, async (req, res) => {
        try {
            ...
            const canonicalId = await resolveCatalogBlankId(String(blankId));
            if (canonicalId === null) {
                res.status(400).json({ error: `Blank "${blankId}" is pending classification and cannot be tier-assigned yet` });
                return;
            }
    ...
    app.post('/admin/catalogs/migrate-blank-ids', middleware_1.requireAdmin, async (req, res) => {
        try {
            ...
            const snap = await core_1.db.collection('catalogs').get();
            const report = [];
            for (const docSnap of snap.docs) {
                const catalogId = docSnap.id;
                const data = docSnap.data();
                const blankIds = (data.blankIds || []).map(String);
                ...
                const resolvedIds = [];
                const dropped = [];
                const remap = new Map(); // oldKey → newKey (null = drop)
                for (const rawId of blankIds) {
                ...
                const updates = { blankIds: newBlankIds, updatedAt: new Date().toISOString() };
                for (const mapField of OVERLAY_MAPS) {
                    const oldMap = data[mapField] || {};
