...
function registerAdminBuildSessions(app) {
            ...
            const sessionData = {
                ...
                working: {
                    ...
                    metadata: {
                        ...
                        fulfillmentProvider: master.fulfillmentProvider ?? (master.printifyBlueprintId ? 'printify' :
    ...
    app.post('/admin/build-sessions/:id/commit', middleware_1.requireAdmin, async (req, res) => {
        try {
            ...
            if (selectedChannel?.id) {
                const chanDoc = await core_1.db.collection('storeChannels').doc(selectedChannel.id).get();
                if (!chanDoc.exists) {
                    res.status(400).json({
            ...
            if (newPacketId) {
                try {
                    const packetSnap = await core_1.db.collection(PRODUCT_PACKETS_COLLECTION).doc(newPacketId).get();
                    if (packetSnap.exists) {
                        const { registerPacketGrfAssets } = await Promise.resolve().then(() => __importStar(require('../services/grf-registrar')));
                        grfIds = await registerPacketGrfAssets(packetSnap.data(), id, newPacketId);
    ...
    app.post('/admin/qrg/allocate', middleware_1.requireAdmin, async (req, res) => {
        try {
            ...
            const qrgIdentityResult = await (0, qrg_instance_allocator_1.allocateQrgInstance)({ qrgBlankId, context: contextCode });
            res.json({
                success: true,
