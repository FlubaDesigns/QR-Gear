 ...
 */
function buildColorSizeFromDoc(p) {
    const qrgVariants = p.qrgVariants || {};
    ...
    for (const [vc, variant] of Object.entries(qrgVariants)) {
        ...
        if (cc) {
            ...
            if (v.colorLabel)
                colorCodesSeen.get(cc).providerLabels.add(v.colorLabel);
        }
        if (sc) {
            if (!sizeCodesSeen.has(sc)) {
                sizeCodesSeen.set(sc, { label: v.sizeLabel || qrgVariantMappings_1.SIZE_LABELS[sc] || sc, providerLabels: new Set() });
            }
            if (v.sizeLabel)
                sizeCodesSeen.get(sc).providerLabels.add(v.sizeLabel);
        }
    }
    const colorMap = Array.from(colorCodesSeen.entries()).map(([code, info]) => {
        ...
        return { qrgColorCode: code, colorName, hex: (0, colorUtils_1.resolveColorHex)(colorName), available: true };
    });
    ...
    app.get('/master-catalog', async (_req, res) => {
        try {
            ...
            for (const doc of snap.docs) {
                ...
                const providerMappings = Array.isArray(p.providerMappings) ? p.providerMappings : [];
                ...
                const availableVia = Array.isArray(p.availableVia) && p.availableVia.length > 0
                    ? p.availableVia
                ...
                const madeInUSA = p.madeInUSA ??
                    providerMappings.some((m) => m.isUSA) ??
            ...
            const result = Object.entries(categories)
                ...
                count: items.length,
                printifyCount: items.filter((i) => i.availableVia?.includes('printify')).length,
                printfulCount: items.filter((i) => i.availableVia?.includes('printful')).length,
                bothCount: items.filter((i) => i.availableVia?.length > 1).length,
            }))
                .sort((a, b) => {
    ...
    app.get('/master-catalog/joint', async (_req, res) => {
        try {
            ...
            for (const doc of masterSnap.docs) {
                ...
                if (!blankKey || !adminDescribedKeys.has(blankKey))
                    continue;
