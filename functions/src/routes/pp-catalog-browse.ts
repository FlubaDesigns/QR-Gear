...
app.get('/master-catalog', async (_req: Request, res: Response): Promise<void> => {
  try {
    ...
    for (const prov of provSnap.docs.map(d => d.data())) {
      ...
      const sizes = Array.isArray(prov.availableSizes) ? prov.availableSizes as string[] : [];
      if (!existing || colors.length > existing.colors.length) {
    ...
    const variantLookup = cfBuildPrintfulVariantLookup(allVariants);
    const printfulByModel = new Map<string, { pfId: number; title: string; brand: string; imageUrl: string | null; madeInUSA: boolean; minPrice: string | null; maxPrice: string | null; colors: Array<{name: string; hex: string}>; sizes: string[] }>();
    for (const doc of pfProductsSnap.docs) {
      const p = { id: parseInt(doc.id) || (doc.data() as any).id, ...doc.data() } as any;
