...
export function register(app: express.Express): void {
  ...
  app.get('/marketplace/amazon/oauth/start', requireAdmin, async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      let oauthUrl: string;
      try {
        oauthUrl = buildOAuthUrl(accountId);
      } catch (err: any) {
        res.status(503).json({
  ...
  app.get('/marketplace/amazon/oauth/callback', async (req: Request, res: Response): Promise<void> => {
    const { spapi_oauth_code, state: accountId, selling_partner_id, error: oauthError } = req.query as Record<string, string>;
    ...
    try {
      ...
      const tokens = await exchangeAuthCodeForTokens(spapi_oauth_code);
      ...
      if (!sellerId || sellerId === 'undefined') {
        try {
          const info = await getSellerIdFromToken(accessToken);
      ...
      if (sellerId) updates.amazonSellerId = sellerId;
      if (marketplaceIds.length > 0) updates.amazonMarketplaceIds = marketplaceIds;
  ...
  app.delete('/admin/surfaces/accounts/:accountId/amazon-disconnect', requireAdmin, async (req: Request, res: Response): Promise<void> => {
    try {
