...
export function registerProductRoutes(app: Express): void {
  ...
  // ── Master catalog purge utility ─────────────────────────────────────────
  // Scans master_catalog and deletes any doc whose ID does not match the
  // canonical qrg_STNNN format (e.g. legacy "71", "1002", "pf:123" docs).
  app.post("/api/admin/master-catalog/purge-non-qrg", isAdmin, async (_req, res) => {
    try {
      const { getFirestoreDb } = await import("../lib/firebase-admin");
      const { isQRGBlankId } = await import("../../shared/blankKeys");
      const fsDb = getFirestoreDb();
      if (!fsDb) return res.status(503).json({ error: "Firestore not available" });
      ...
      for (const doc of snap.docs) {
        const id = doc.id;
        if (id.startsWith('qrg_') && isQRGBlankId(id)) {
          kept.push(id);
        } else {
          console.log(`[purge-non-qrg] Deleting non-QRG doc: ${id}`);
      ...
      console.log(`[purge-non-qrg] Deleted ${toDelete.length} non-QRG docs, kept ${kept.length} QRG docs`);
      res.json({ deleted: toDelete.length, kept: kept.length, deletedIds: toDelete.map(r => r.id) });
    } catch (error: any) {
      console.error("[purge-non-qrg] Error:", error);
      res.status(500).json({ error: error.message });
    }
  });
